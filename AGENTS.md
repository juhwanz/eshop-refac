...
- 장기간 유지될 아키텍처, 데이터 정합성, 보안, 운영 방식, 주요 의존성에 관한 결정은 ADR 후보로 판단한다.
- ADR 후보가 생기면 결정 배경, 검토한 대안, 장단점을 요약해 사용자에게 기록 여부를 제안한다.
- 사용자가 기록을 승인하면 추가 지시를 기다리지 않고 `docs/adr/README.md`의 규칙과 형식에 따라 ADR을 생성한다.
- ADR 생성 시 `docs/adr/`를 만들고 기존 번호 다음의 4자리 일련번호와 짧은 kebab-case 제목을 사용한다. 디렉터리가 이미 있으면 기존 파일을 보존한다.
...
- `공부 시작`, 복습, 모의 면접, 프로젝트 설명 연습 요청에는 저장소 Skill인 `eshop-study`를 활용한다.
- 진도와 학습 기록은 `docs/learning/`을 기준으로 하며, 구체적인 진행·기록 규칙과 변경 경계는 해당 Skill을 따른다.
